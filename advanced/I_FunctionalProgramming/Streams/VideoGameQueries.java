package pl.sdacademy.java.advanced.I_FunctionalProgramming.Streams;
...
public class VideoGameQueries {
    ...
    public static Optional<VideoGame> getByTitle(String title) {
        ...
        */
        ...
        return games.stream().filter(videoGame -> videoGame.getTitle().equals(title)).findFirst();
    }

    // return all games from given publisher
    public static List<VideoGame> getByPublisher(String publisher) {
       /* List<VideoGame> games = new ArrayList<>();
        for (VideoGame game : VideoGameQueries.games) {
...
//        return games.stream().max(Comparator.comparingDouble(g -> g.getSoldUnitsInMillions()));
    }
    ...
    public static void resetAllUnitsSoldValues() {
        games.forEach(game -> game.setSoldUnitsInMillions(0.0+1));
    }
    ...
    public static void optionalIfPresentExample(String title) {
        /* Implementation with if-else statement
        Optional<VideoGame> game = getByTitle(title);
        if (game.isPresent()) {
